                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            $sql = "SELECT * FROM detail_transaksi ORDER BY id_trx DESC";
                            $qry = mysqli_query($koneksi, $sql);

                            while ($a = mysqli_fetch_array($qry)) {
                                echo "<tr>";
                                echo "<td><input type='checkbox' name='selected_ids[]' value='{$a['id_trx']}'></td>";
                                echo "<td>{$a['id_trx']}</td>";
                                echo "<td>{$a['id_transaksi']}</td>";
                                echo "<td>{$a['id_barang']}</td>";
                                echo "<td>{$a['jumlah']}</td>";
                                echo "<td>{$a['sub_total']}</td>";
                                echo "<td>
                                        <a href=\"edit.php?id_trx={$a['id_trx']}\" class=\"btn btn-warning btn-sm\" title=\"Edit\">
                                            <i class=\"fas fa-edit\"></i>
                                        </a>
                                          <a href=\"hapus.php?id_trx={$a['id_trx']}\" class=\"btn btn-danger btn-sm\" title=\"Hapus\" onclick=\"return confirm('Yakin ingin menghapus data ini?')\"> <!-- Disesuaikan jalur -->
