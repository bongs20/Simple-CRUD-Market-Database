                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            $sql = "SELECT * FROM laporan_keuangan ORDER BY id_laporan DESC";
                            $qry = mysqli_query($koneksi, $sql);

                            while ($a = mysqli_fetch_array($qry)) {
                                echo "<tr>";
                                echo "<td><input type='checkbox' name='selected_ids[]' value='{$a['id_laporan']}'></td>";
                                echo "<td>{$a['id_laporan']}</td>";
                                echo "<td>{$a['periode']}</td>";
                                echo "<td>{$a['total_pemasukan']}</td>";
                                echo "<td>{$a['total_pengeluaran']}</td>";
                                echo "<td>{$a['keuntungan']}</td>";
                                echo "<td>
                                        <a href=\"edit.php?id_laporan={$a['id_laporan']}\" class=\"btn btn-warning btn-sm\" title=\"Edit\">
                                            <i class=\"fas fa-edit\"></i>
                                        </a>
                                            <a href=\"hapus.php?id_laporan={$a['id_laporan']}\" class=\"btn btn-danger btn-sm\" title=\"Hapus\" onclick=\"return confirm('Yakin ingin menghapus data ini?')\"> <!-- Disesuaikan jalur -->
