...
<body class="bg-light">
    ...
    <ul class="dropdown-menu">
        ...
        <li><a class="dropdown-item" href="../pemasok/index.php">Detail Transaksi</a></li>
        ...
        <li><a class="dropdown-item" href="../pembelian_stok/index.php">Pembelian Stok</a></li>
            ...
            <h1 class="text-center mb-4">Pemasok</h1>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            $sql = "SELECT * FROM pemasok ORDER BY id_pemasok DESC";
                            $qry = mysqli_query($koneksi, $sql);

                            while ($a = mysqli_fetch_array($qry)) {
                                echo "<tr>";
                                echo "<td><input type='checkbox' name='selected_ids[]' value='{$a['id_pemasok']}'></td>";
                                echo "<td>{$a['id_pemasok']}</td>";
                                echo "<td>{$a['nama_pemasok']}</td>";
                                echo "<td>{$a['no_telepon']}</td>";
                                echo "<td>{$a['email']}</td>";
                                echo "<td>{$a['alamat']}</td>";
                                echo "<td>{$a['kode_pos']}</td>";
                                echo "<td>{$a['kota']}</td>";
                                echo "<td>
                                        <a href=\"edit.php?id_pemasok={$a['id_pemasok']}\" class=\"btn btn-warning btn-sm\" title=\"Edit\">
                                            <i class=\"fas fa-edit\"></i>
                                        </a>
                                            <a href=\"hapus.php?id_pemasok={$a['id_pemasok']}\" class=\"btn btn-danger btn-sm\" title=\"Hapus\" onclick=\"return confirm('Yakin ingin menghapus data ini?')\"> <!-- Disesuaikan jalur -->
